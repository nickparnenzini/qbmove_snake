...
{ 
            ...
            std::string string_plugin = "model_qbmove_" + boost::to_string(stat_value);      

            this->num_qbmove = stat_value; 

            std::string joint_name = "";
            std::string submodel_name = "";

            submodel_name = "qbmove_" + boost::to_string(this->num_qbmove);

            joint_name = submodel_name + "::output_shaft_joint"; 

            this->joint = this->model->GetJoint(joint_name); 

            joint_name = submodel_name + "::motor1_joint"; 

            this->joint_motor1 = this->model->GetJoint(joint_name); 

            joint_name = submodel_name + "::motor2_joint"; 

            this->joint_motor2 = this->model->GetJoint(joint_name);  

            /* strings used to define stiffness and position topics for Gazebo commands */
            std::string command_topic =  submodel_name + "/position_stiffness/command";

            std::string echo_topic =   submodel_name + "/pos_current/echo";

/*******/            
            std::cout << command_topic << "\n";
            std::cout << echo_topic << "\n"; 
